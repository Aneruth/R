...
X_train = as.matrix(data[,1:35])
X_test = as.matrix(data[,1:35])
y_train = as.matrix(data[,36])
y_test = as.matrix(data[,36])
...
mod <- Sequential()
mod$add(Dense(units = n_units, input_shape = dim(X_train)))
mod$add(LeakyReLU())
